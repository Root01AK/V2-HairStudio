...
export default function OurProcessRotating() {
  ...
  // GSAP for steps strip animations
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Initial stagger animation for strip cards
      gsap.fromTo(
        ".process-strip-card",
        { 
          opacity: 0, 
  ...
  // Handle card interactions with GSAP
  const handleCardHover = (card, isHover) => {
    const content = card.querySelector(".process-step-content");
    const title = card.querySelector(".process-step-title");
    const label = card.querySelector(".process-step-label");
    const hint = card.querySelector(".hover-hint");
  ...
  return (
    <>
      <section ref={sectionRef} className="process-rotating-section">
        ...
        {/* STEPS STRIP WITH GSAP */}
        <div ref={stripRef} className="process-steps-strip">
